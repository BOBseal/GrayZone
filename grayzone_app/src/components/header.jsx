...
const Header = () => {
    const {connectWallet, user , isPassholder} = useContext(DappAppContext);
   // const [x, setX ] = useState(false); // menu opener
    ...
    async function userHandle () {
        try {
            console.log(p)
            const x = await isPassholder();
            if(x > 0 ) {
                router.push('/dashboard')
            } else { 
                router.push('/mintpass');
            }
        } catch (error) {
            console.log(error)
        }
        ...
        <div className='h-full w-full flex justify-between gap-1 text-[#FFFFFF]'>
            ...
            <div className='right flex justify-end md:pr-[2.5rem] lg:pr-[2.9rem] items-center w-7/12'>
                ...
                >
                    ...
                    {controllers .menu ? 
                    <div className='absolute top-[4.23rem] md:top-[4.5rem] w-[13rem] z-50 border shadow-xl right-0 bg-gradient-to-r to-[#b67ef5] from-[#1D023C] from-10% to-85% rounded-lg p-8 md:hidden lg:hidden pt-4 pb-4 pr-[0.9rem] items-center justify-center' >
                        
                        <div className='flex flex-col justify-center items-center gap-2 p-4 drop-shadow-lg'>
                            <select className={` bg-[#1d023cb1] rounded-full p-1`}>
                                    <option>LIN</option>
                                    <option>BASE</option>
                                    <option>MNT</option>
                                    <option>ARB</option>
                            ...
                            { !user.str ? <button onClick={()=>connectWallet()} className="drop-shadow-lg">
                                    LOGIN
                                </button> :
                                <div className='flex text-[1rem] cursor-pointer' onClick={()=>userHandle()}>
                                    { user.str}
                                </div>
                            }
                    ...
                    <div className={`hidden md:flex justify-between gap-4 mt-2 w-11/12 font-semibold text-[1.1rem]`}>
                        ...
                        <div className='w-[6rem] flex justify-center items-center drop-shadow-lg'>
                        ...
                        <select className={` bg-[#b67ef5] rounded-full p-1`}>
                                    <option className=''>Linea</option>
                                    <option>Base</option>
                                    <option>Mantle</option>
                                    <option>Arbitrum</option>
                                    <option>Manta</option>
                                    <option>Taiko Jolnir</option>
                                    <option>Linea Test</option>
                        </select>
